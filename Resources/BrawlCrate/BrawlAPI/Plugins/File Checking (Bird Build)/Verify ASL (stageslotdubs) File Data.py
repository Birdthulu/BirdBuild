...
def main():
	...
	# Prompt for the stageslotdubs directory
	workingDir = BrawlAPI.OpenFolderDialog("Open pf or stageslotdubs folder")
	if not workingDir:
		return
	
	if str(workingDir).endswith("\\pf"):
		workingDir += "\\stage\\stageslotdubs\\"
	elif str(workingDir).endswith("\\pf\\stage"):
		workingDir += "\\stageslotdubs\\"
	else:
		workingDir += "\\"
	...
	# Derive param folder
	PARAM_DIR_PATH = str(workingDir).rsplit("\\",2)[0] + "\\stageinfodubs"
	
	# Open whole stageslotdubs folder in BrawlCrate
