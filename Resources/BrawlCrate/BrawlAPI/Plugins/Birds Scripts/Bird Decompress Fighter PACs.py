...
def decompressFile():
    ...
    if isinstance(BrawlAPI.RootNode, ARCNode):
        # Compress as None if it isn't already
        if not BrawlAPI.RootNode.Compression == "None":
            BrawlAPI.RootNode.Compression = "None"
            BrawlAPI.SaveFile()
...
if folder:
    ...
    for subfolder in fighterDir:
        ...
        if ("kirby" in name) or ("koopa" in name) or ("gkoopa" in name) or ("zelda" in name) or ("sheik" in name) or ("poketrainer" in name):
            for file in subfolder.GetFiles():
                ...
                if re.search(r"(?<![Ee][Tt][Cc])(?<![Ff][Ii][Nn][Aa][Ll])(?<![Rr][Ee][Ss][Uu][Ll][Tt])(?<![Ee][Nn][Tt][Rr][Yy])(\d\d|[Ss][Pp][Yy]|[Aa][Ll][Tt]\w).[Pp][Aa][Cc]$", file.Name) is not None:
                    decompressFile()
        if ("mario" in name) or ("pit" in name):
            for file in subfolder.GetFiles():
                # Get ##, Spy, and Alt costumes that aren't Etc/Final/Results/Entry files
                if re.search(r"(?<![Ee][Tt][Cc])(?<![Ff][Ii][Nn][Aa][Ll])(?<![Rr][Ee][Ss][Uu][Ll][Tt])(?<![Ee][Nn][Tt][Rr][Yy])(\d\d|[Ss][Pp][Yy]|[Aa][Ll][Tt]\w).[Pp][Aa][Cc]$", file.Name) is not None:
                    if ("fitmario03.pac" in file.Name.lower()) or ("fitpit01.pac" in file.Name.lower()):
                        decompressFile()
