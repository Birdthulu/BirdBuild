...
# Run from MenuRule ARC - MenuRule in name and parent is named db_menumain
# Wrapper type: ARCWrapper
def EnableCheckMenuRuleARC(sender, event_args):
	node = BrawlAPI.SelectedNode
	sender.Enabled = node is not None and "MenuRule" in node.Name and node.Parent and "db_menumain" in node.Parent.Name

# Run from root menumain ARC 
def EnableCheckMenumainARC(sender, event_args):
	node = BrawlAPI.SelectedNode
	sender.Enabled = node is not None and "db_menumain" in node.Name and node.Children and getChildFromName(node, "MenuRule_")

# Run from selcharacter2 root ARC
def EnableCheckSelchar2RootARC(sender, event_args):
	node = BrawlAPI.SelectedNode
	sender.Enabled = node is not None and "db_selcharacter2_" in node.Name and node.Children and getChildFromName(node,"MenuRule_")

# Run from selcharacter2 MenuRule ARC
def EnableCheckSelchar2MenuruleARC(sender, event_args):
	node = BrawlAPI.SelectedNode
	sender.Enabled = node is not None and "MenuRule" in node.Name and node.Parent and "db_selcharacter2_" in node.Parent.Name
...
def main_menumain(node):
	...
	MENU2_FOLDER = str(MENUMAIN_PATH).split("db_menumain.pac")[0]
	SELCHARACTER2_PATH = MENU2_FOLDER + "\db_selcharacter2.pac"
	message = "Exporting MenuRule_en as a db_selcharacter2.pac file inside\n" + MENU2_FOLDER
...
def main_selchar2(node):
	...
	message = "Exporting MenuRule_en into the db_menumain.pac file\n" + MENU2_FOLDER
	message += "\n\nPress OK to continue."
...
BrawlAPI.AddContextMenuItem(ARCWrapper, "", "Export MenuRule ARC as a db_selcharacter2.pac", EnableCheckMenuRuleARC, ToolStripMenuItem("Export as db_selcharacter2", None, export_menumain_menurule))

# From menumain (root node) ARC
BrawlAPI.AddContextMenuItem(ARCWrapper, "", "Export MenuRule ARC as a db_selcharacter2.pac", EnableCheckMenumainARC, ToolStripMenuItem("Export MenuRule as db_selcharacter2", None, export_menumain_rootnode))

# From selchar2 (root node) ARC
BrawlAPI.AddContextMenuItem(ARCWrapper, "", "Export db_selcharacter2 file to MenuRule ARC in menumain .pac", EnableCheckSelchar2RootARC, ToolStripMenuItem("Export into db_menumain", None, export_selchar2_rootnode))

# From selchar2 > MenuRule ARC
BrawlAPI.AddContextMenuItem(ARCWrapper, "", "Export db_selcharacter2 file to MenuRule ARC in menumain .pac", EnableCheckSelchar2MenuruleARC, ToolStripMenuItem("Export into db_menumain", None, export_selchar2_MenuRule))
