...
def main():
	...
	# Prompt for the stageinfone directory
	workingDir = BrawlAPI.OpenFolderDialog("Open pf or stageinfone folder")
	if not workingDir:
		return

	if workingDir.endswith("\\pf"):
		workingDir += "\\stage\\stageinfone\\"
	elif workingDir.endswith("\\pf\\stage"):
		workingDir += "\\stageinfone\\"
	else:
		workingDir += "\\"
	...
	# Open whole stageinfone folder in BrawlCrate
