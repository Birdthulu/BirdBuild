...
def EnableCheckARC(sender, event_args):
	sender.Enabled = (BrawlAPI.SelectedNode is not None and "MenuRule" in BrawlAPI.SelectedNode.Name and BrawlAPI.SelectedNode.Parent and "sc_selcharacter2" in BrawlAPI.RootNode.FilePath)
...
def export_menurule(sender, event_args):
	...
	message = "Exporting sc_selcharacter2.pac as a MenuRule_en file inside\n" + MENU2_FOLDER
...
BrawlAPI.AddContextMenuItem(ARCWrapper, "", "Export archive as a MenuRule_en", EnableCheckARC, ToolStripMenuItem("Export to mu_menumain", None, export_menurule))
