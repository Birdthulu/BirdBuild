...
def main():
	...
	# Prompt for the stageslotne directory
	workingDir = BrawlAPI.OpenFolderDialog("Open pf or stageslotne folder")
	if not workingDir:
		return
	
	if str(workingDir).endswith("\\pf"):
		workingDir += "\\stage\\stageslotne\\"
	elif str(workingDir).endswith("\\pf\\stage"):
		workingDir += "\\stageslotne\\"
	else:
		workingDir += "\\"
	...
	# Derive param folder
	PARAM_DIR_PATH = str(workingDir).rsplit("\\",2)[0] + "\\stageinfone"
	
	# Open whole stageslotne folder in BrawlCrate
