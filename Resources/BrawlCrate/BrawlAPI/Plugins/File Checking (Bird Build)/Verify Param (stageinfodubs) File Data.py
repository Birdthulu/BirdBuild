...
def main():
	...
	# Prompt for the stageinfodubs directory
	workingDir = BrawlAPI.OpenFolderDialog("Open pf or stageinfodubs folder")
	if not workingDir:
		return

	if workingDir.endswith("\\pf"):
		workingDir += "\\stage\\stageinfodubs\\"
	elif workingDir.endswith("\\pf\\stage"):
		workingDir += "\\stageinfodubs\\"
	else:
		workingDir += "\\"
	...
	# Open whole stageinfodubs folder in BrawlCrate
